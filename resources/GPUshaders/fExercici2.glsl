...
void main()
{
    // Calculem el radi actual tenint en compte el temps
    float radius = max(MAX_RADIUS - time * SPEED, MIN_RADIUS);

    // Gouraud si estem fora de l'esfera
    if (length(P-CENTER) > radius) {
        // La conversió a grisos es fa al vertex shader
        colorOut = clamp(colorGouraud, 0, 1);
    } 
    
    // Cell shader si estem a l'esfera (o justament a la frontera)
    else {
