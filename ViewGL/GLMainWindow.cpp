    ...
    ui(new Ui::MainWindow)
{
    ...
    setWindowTitle(tr("Pràctica 2: ZbufferToy - 2023-24"));
    changes = true;
    ...
    connect(ui->action_genera_escena, SIGNAL(triggered()), builder, SLOT(newSimulatedScene()));
    ...
    connect(builder, SIGNAL(settingsChanged()), glWidget, SLOT(updateSettings()));
...
void GLMainWindow::refreshWindow() {
    auto cam = Controller::getInstance()->getSetUp()->getCamera();

    // Actualització plans de clipping a la GUI   
    ui->persFarSpin->setValue(cam->dpost);
    ui->persNearSpin->setValue(cam->dant);
    ...
    // Actualització observador a la GUI
    ui->lookEyeXSpin->setValue((float)cam->origin.x);
    ui->lookEyeYSpin->setValue((float)cam->origin.y);
    ui->lookEyeZSpin->setValue((float) cam->origin.z);
    ...
    // TODO: Modificar pestanya de llums al carregar SetUp des de fitxer!
}
...
void GLMainWindow::connectShaders()
{
    ...
    connect(this->ui->action_indirect_mapping, SIGNAL(triggered()), glWidget, SLOT(activaIndirectMapping()));

    connect(this->ui->action_exercici1, SIGNAL(triggered()), glWidget, SLOT(activaExercici1()));
    connect(this->ui->action_exercici2, SIGNAL(triggered()), glWidget, SLOT(activaExercici2()));
